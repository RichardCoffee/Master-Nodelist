...
abstract class WMN_Plugin_Plugin {

	protected $admin    = null;
	public    $dbvers   = '0';
	protected $github   = '';    #  'https://github.com/MyGithubName/my-plugin-name/';
	public    $paths    = null;  #  WMN_Plugin_Paths object
	...
	protected function __construct( $args = array() ) {
		if ( ! empty( $args['file'] ) ) {
			...
			$this->paths = WMN_Plugin_Paths::get_instance( $args );
	...
	public function settings_link( $links, $file, $data, $context ) {
		if ( strpos( $file, $this->plugin ) !== false ) {
			...
			if ( is_plugin_active( $file ) ) {
				...
				$links['settings'] = sprintf( '<a href="%s"> %s </a>', esc_url( $url ), esc_html__( 'Settings', 'wmn-workbook' ) );
			}
		}
		return $links;
	}
