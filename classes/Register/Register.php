...
class WMN_Register_Register {
	...
	public static function php_version_check() {
		if ( version_compare( phpversion(), static::php_version_required(), '<' ) ) {
			add_action( 'admin_notices', array( 'WMN_Register_Register', 'unsupported_php_version' ), 10, 2 );
			return false;
		}
		return true;
	}
	...
	public static function unsupported_php_version() {
		$short = __( 'You are running an unsupported version of PHP.', 'wmn-workbook' );
	...
	public static function wp_version_check() {
		if ( version_compare( $GLOBALS['wp_version'], static::wp_version_required(), '<' ) ) {
			add_action( 'admin_notices', array( 'WMN_Register_Register', 'unsupported_wp_version' ), 10, 2 );
			return false;
		}
		return true;
	}
	...
	public static function unsupported_wp_version() {
		$short = __( 'You are running an unsupported version of WordPress.', 'wmn-workbook' );
	...
	private static function display_admin_notice( $short, $long ) {
		...
		<div class="notice notice-error">
			<p style="max-width:800px;">
				<b><?php echo esc_html( sprintf( _x( '%s can not be activated.', 'Plugin title', 'wmn-workbook' ), static::$title ) );?></b>
